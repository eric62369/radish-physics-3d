...
func test_deterministic_rotation():
	...
	t = SGFixed.transform3d(0, SGFixed.vector3(0, 0, 0))
	var target = SGFixed.vector3(65536, 0, 0)
	t = t.looking_at(target, true)
	assert_eq(t.x.x, 0)
	assert_eq(t.x.y, 0)
	assert_eq(t.x.z, -65536)
	assert_eq(t.y.x, 0)
	assert_eq(t.y.y, 65536)
	assert_eq(t.y.z, 0)
	assert_eq(t.z.x, target.x)
	assert_eq(t.z.y, target.y)
	assert_eq(t.z.z, target.z)

	t = SGFixed.transform3d(0, SGFixed.vector3(0, 0, 0))
	target = SGFixed.vector3(65536, 0, 65536)
	t = t.looking_at(target, true)
	target = target.normalized()
	assert_eq(t.x.x, 46341)
	assert_eq(t.x.y, 0)
	assert_eq(t.x.z, -46341)
	assert_eq(t.y.x, 0)
	assert_eq(t.y.y, 65537)
	assert_eq(t.y.z, 0)
	assert_eq(t.z.x, target.x)
	assert_eq(t.z.y, target.y)
	assert_eq(t.z.z, target.z)
