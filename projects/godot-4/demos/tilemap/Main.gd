...
func _exit_tree():
	for rid in resources:
		SGPhysics3DServer.free_rid(rid)
...
func create_tilemap_collision(tilemap: TileMap, colliding_tiles: Array) -> void:
	...
	for tile_id in colliding_tiles:
		for tile_pos in tilemap.get_used_cells_by_id(0, tile_id):
			var shape = SGPhysics3DServer.shape_create(SGPhysics3DServer.SHAPE_RECTANGLE)
			SGPhysics3DServer.rectangle_set_extents(shape, extents)
			SGPhysics3DServer.shape_set_transform(shape, shape_transform)
			resources.push_back(shape)

			var object = SGPhysics3DServer.collision_object_create(SGPhysics3DServer.OBJECT_BODY, SGPhysics3DServer.BODY_STATIC)
			SGPhysics3DServer.collision_object_add_shape(object, shape)
			SGPhysics3DServer.collision_object_set_transform(object, SGFixed.transform2d(0,
			...
			SGPhysics3DServer.collision_object_set_data(object, "tilemap-%sx%s" % [int(tile_pos.x), int(tile_pos.y)])
			resources.push_back(object)

			SGPhysics3DServer.world_add_collision_object(SGPhysics3DServer.get_default_world(), object)
